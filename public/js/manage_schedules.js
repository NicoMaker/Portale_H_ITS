...
function renderSchedules() {
  ...
  if (!filtered.length) {
    ...
    filtered.forEach((s) => {
      ...
      html += `<tr>`;
      html += `<td><span class='badge' style='background:#f1f5f9;color:var(--primary);'>${
        course ? course.name : ""
      }</span></td>`;
...
function renderSchedulesList() {
  ...
  if (!filtered.length) {
    ...
    filtered.forEach((s) => {
      const course = allCourses.find((c) => c.id == s.course_id) || {};
      html += `<tr><td>${course.name || "-"}</td><td>${s.teacher}</td><td>${
        s.room
      }</td><td>${s.subject || ""}</td><td>${s.day}</td><td>${
        typeof formatDate === "function" ? formatDate(s.date) : s.date
      }</td><td>${s.start_time}</td><td>${
        s.end_time
      }</td><td style='text-align:center;'><button class='icon-btn' title='Modifica' onclick='openEditSchedule(${
        s.id
      })'>✏️</button> <button class='icon-btn' title='Elimina' onclick='deleteSchedule(${
        s.id
      })'>🗑️</button></td></tr>`;
    });
    html += "</tbody></table></div>";
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      if (end <= start) {
        e.preventDefault();
        alert(
          "Non è possibile inserire orario perché ore di fine maggiore di ore di inizio",
        );
        return false;
      }
    });
