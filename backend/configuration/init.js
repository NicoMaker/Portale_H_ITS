...
function randomUsername(role) {
  const firstNames = [
    ...
    "Paolo", "Laura", "Giorgio", "Valentina", "Andrea", "Martina", "Davide",
    "Simona", "Pietro", "Alice", "Matteo", "Isabella", "Antonio", "Francesca",
    "Giuseppe", "Anna", "Alessandro", "Maria", "Roberto", "Giulio", "Claudia",
    "Stefano", "Silvia", "Federico", "Elisabetta", "Riccardo", "Cristina",
  ];
  const lastNames = [
    "Rossi", "Bianchi", "Verdi", "Neri", "Gialli", "Blu", "Viola", "Grigi",
    "Marroni", "Argento", "Ferrari", "Conti", "De Luca", "Lombardi", "Moretti",
    "Marchetti", "Serra", "Gentile", "Barbieri", "Santoro", "Romano", "Gallo",
    "Costa", "Ricci", "Fontana", "Esposito", "Russo", "Villa", "Colombo",
    "Marino", "Greco", "Bruno", "Galli", "Leone", "Longo", "Giordano", "Mancini",
  ];
...
function randomCourseName() {
  const topics = [
    "Informatica", "Ingegneria del Software", "Data Science", "Cybersecurity",
    "Web Development", "Intelligenza Artificiale", "Blockchain", "Cloud Computing",
    "UX/UI Design", "Game Development", "Big Data Analytics", "Robotica", "IoT",
    "Bioinformatica", "Machine Learning", "DevOps", "Mobile Development",
    "Computer Graphics", "Networking", "Database Management", "Software Testing",
    "Computer Vision", "Natural Language Processing", "Quantum Computing",
    "Algoritmi Avanzati", "Sicurezza delle Reti", "Programmazione Web",
    "Analisi dei Dati", "Sistemi Distribuiti", "Architettura Software",
  ];
  const levels = ["Base", "Avanzato", "Specialistico", "Master", "Triennale", "Magistrale", "Professionale"];
  const randomNum = Math.floor(Math.random() * 9999) + 1;
  return `${getRandomItem(topics)} ${getRandomItem(levels)} ${randomNum}`;
}

function getRandomTimeAndLocation() {
  const teachers = ["Prof. Smith", "Prof. Johnson", "Prof. Garcia", "Prof. Miller", "Prof. Davis", "Prof. Rossi", "Prof. Bianchi", "Prof. Verdi", "Prof. Neri", "Prof. Bruno", "Prof. Ferrari", "Prof. Romano", "Prof. Gallo", "Prof. Conti", "Prof. Ricci", "Prof. Lombardi", "Prof. Moretti", "Prof. Marchetti", "Prof. Serra", "Prof. Gentile", "Prof. Barbieri", "Prof. Santoro", "Prof. Costa", "Prof. Fontana", "Prof. Esposito", "Prof. Villa", "Prof. Colombo", "Prof. Marino", "Prof. Greco", "Prof. Leone", ];
  const rooms = ["Aula A101", "Aula A102", "Aula A103", "Aula B201", "Aula B202", "Aula B203", "Lab Informatica 1", "Lab Informatica 2", "Lab Informatica 3", "Aula C301", "Aula C302", "Aula C303", "Lab Software", "Lab AI", "Lab Machine Learning", "Aula D401", "Aula D402", "Aula D403", "Lab Sicurezza", "Lab Networking", "Aula E501", "Aula E502", "Lab Robotica", "Lab Database", "Lab Cloud", "Aula Magna", "Sala Conferenze A", "Sala Conferenze B", "Aula F601", "Aula F602", ];
...
async function populateDatabase() {
  try {
    ...
    let numAdmins, numStudentsEnrolled, numStudentsUnenrolled, numCoursesWithStudents, numCoursesWithoutStudents, schedulesPerCourse;
    do {
      numAdmins = parseInt(await ask("Quanti admin casuali vuoi generare (oltre ad Admin principale)? (0-100): "));
    } while (isNaN(numAdmins) || numAdmins < 0 || numAdmins > 100);
    do {
      numStudentsEnrolled = parseInt(await ask("Quanti studenti vuoi iscrivere a un corso? (0-1000): "));
    } while (isNaN(numStudentsEnrolled) || numStudentsEnrolled < 0 || numStudentsEnrolled > 1000);
    do {
      numStudentsUnenrolled = parseInt(await ask("Quanti studenti vuoi che NON siano iscritti a un corso? (0-1000): "));
    ...
    do {
      schedulesPerCourse = parseInt(await ask("Quanti orari per corso (anche quelli senza studenti)? (1-200): "));
    } while (isNaN(schedulesPerCourse) || schedulesPerCourse < 1 || schedulesPerCourse > 200);
    rl.close();
    ...
    await runQuery("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", ["Admin", adminHash, "admin"]);
    for (let i = 0; i < numAdmins; i++) {
      const uname = randomUsername("admin");
      const hash = await hashPassword("Admin123!");
      await runQuery("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", [uname, hash, "admin"]);
    }
    ...
    for (let i = 0; i < numStudentsEnrolled; i++) {
      ...
      const result = await runQuery("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", [uname, hash, "user"]);
      studentsEnrolledIds.push(result.lastID);
    }
    for (let i = 0; i < numStudentsUnenrolled; i++) {
      const uname = randomUsername("user");
      const hash = await hashPassword("User123!");
      await runQuery("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", [uname, hash, "user"]);
    }

    console.log("📚 Creazione corsi...");
    const coursesWithStudentsIds = [];
    for (let i = 0; i < numCoursesWithStudents; i++) {
      const result = await runQuery("INSERT INTO courses (name, description) VALUES (?, ?)", [randomCourseName(), "Descrizione del corso generato automaticamente per il sistema di gestione accademico. Include teoria e pratica."]);
      coursesWithStudentsIds.push(result.lastID);
    }
    for (let i = 0; i < numCoursesWithoutStudents; i++) {
      await runQuery("INSERT INTO courses (name, description) VALUES (?, ?)", [randomCourseName(), "Descrizione del corso generato automaticamente per il sistema di gestione accademico. Include teoria e pratica."]);
    }
    ...
    if (studentsEnrolledIds.length > 0 && coursesWithStudentsIds.length > 0) {
      for (let i = 0; i < studentsEnrolledIds.length; i++) {
        ...
        await runQuery("INSERT INTO user_courses (user_id, course_id) VALUES (?, ?)", [studentsEnrolledIds[i], selectedCourseId]);
      }
    ...
    const subjects = ["Programmazione", "Algoritmi e Strutture Dati", "Database", "Machine Learning", "Sicurezza Informatica", "Frontend Development", "Backend Development", "Gestione Progetti", "Cloud Computing", "DevOps", "Data Analysis", "Computer Graphics", "Networking", "Software Engineering", "AI Ethics", "Cybersecurity", "Web Design", "Mobile Programming", "Game Design", "Computer Vision", "NLP", "Blockchain", "IoT", "Robotica", "Big Data", "UX Design", "System Administration", "Software Testing", "Quantum Computing", ];
    ...
    for (const course of allCourses) {
      for (let j = 0; j < schedulesPerCourse; j++) {
        ...
        try {
          ...
          console.error(`❌ Errore durante l'inserimento dell'orario per il corso ID ${course.id}:`, error.message, "Dati orario:", timeAndLocation);
    ...
    const totalCourses = await getAllQuery("SELECT COUNT(*) as count FROM courses");
    const totalSchedules = await getAllQuery("SELECT COUNT(*) as count FROM schedules");
    const totalEnrollments = await getAllQuery("SELECT COUNT(*) as count FROM user_courses");
    ...
    console.log(`   • Admin/Docenti: ${adminsCount[0].count} (incluso Admin principale)`);
    ...
    console.log(`🔗 Iscrizioni totali: ${totalEnrollments[0].count} (MAX 1 corso per studente)`);
    console.log(`🕒 Orari totali: ${totalSchedules[0].count} (${schedulesPerCourse} per corso)`);
    ...
    distributionCheck.forEach((course, index) => {
      ...
      console.log(`${courseNumber}. ${course.course_name.substring(0, 40).padEnd(40)} [${studentCount} studenti]`);
    });

    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("\n🔑 Credenziali predefinite:");
    console.log("   👨‍💼 Admin principale: username='Admin', password='Admin123!'");
