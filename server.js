...
function serveStatic(req, res) {
  ...
  if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
    ...
    const mime = {
      '.html': 'text/html', '.css': 'text/css', '.js': 'application/javascript', '.png': 'image/png', '.jpg': 'image/jpeg', '.ico': 'image/x-icon'
    };
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && parsedUrl.pathname === '/login') {
    parseBody(({username, password}) => {
      db.get('SELECT * FROM users WHERE username = ?', [username], (err, user) => {
        ...
        res.writeHead(302, { Location: user.role === 'admin' ? '/admin_dashboard.html' : '/user_dashboard.html' });
        res.end();
      });
    });
    return;
  ...
  if (req.url.startsWith('/api/users')) {
    ...
    if (req.method === 'GET' && parsedUrl.pathname === '/api/users') {
      db.all('SELECT * FROM users', [], (err, users) => {
        ...
        let done = 0;
        users.forEach(u => {
          db.all('SELECT c.* FROM courses c JOIN user_courses uc ON c.id=uc.course_id WHERE uc.user_id=?', [u.id], (e, courses) => {
            u.courses = courses || [];
    ...
    // DELETE /api/users/:id
    if (req.method === 'DELETE' && parsedUrl.pathname.match(/^\/api\/users\/\d+$/)) {
      const id = parsedUrl.pathname.split('/')[3];
    ...
    if (req.method === 'POST' && parsedUrl.pathname.match(/^\/api\/users\/\d+\/demote$/)) {
      const id = parsedUrl.pathname.split('/')[3];
      db.get('SELECT * FROM users WHERE id=?', [id], (err, user) => {
        if (!user || user.role !== 'admin') { res.writeHead(400); res.end('Non è un admin'); return; }
        db.get('SELECT COUNT(*) as n FROM users WHERE role="admin"', [], (e, row) => {
          if (row && row.n <= 1) {
  ...
  if (req.method === 'GET' && parsedUrl.pathname === '/user/courses') {
    ...
    db.all('SELECT c.* FROM courses c JOIN user_courses uc ON c.id=uc.course_id WHERE uc.user_id=?', [session.user.id], (err, courses) => {
      if (err) { res.writeHead(500); res.end('DB error'); return; }
      res.end(JSON.stringify(courses));
    });
    return;
  ...
  if (req.method === 'GET' && parsedUrl.pathname === '/user/schedules') {
    ...
    db.all('SELECT s.*, c.name as course_name FROM schedules s JOIN user_courses uc ON s.course_id=uc.course_id AND uc.user_id=? LEFT JOIN courses c ON s.course_id=c.id', [session.user.id], (err, schedules) => {
      if (err) { res.writeHead(500); res.end('DB error'); return; }
      res.end(JSON.stringify(schedules));
    });
    return;
