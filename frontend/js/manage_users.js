...
function toggleFilterCourseVisibility() {
  ...
  const group = document.getElementById('filter-course').closest('.filter-group');
  if (filterRole === 'admin') {
...
function showChangeRoleModal(userId, newRole) {
  ...
  document.getElementById('change-role-modal-title').textContent = `Cambia ruolo — ${user.username}`;
...
function changeUserRole() {
  ...
  const endpoint = userToChangeRole === 'admin'
  ...
  fetch(endpoint, { method: 'POST' })
    ...
    .then(() => { hideChangeRoleModal(); fetchUsers(); })
    .catch(err => { console.error(err); hideChangeRoleModal(); });
}
...
document.getElementById('confirm-delete-user').onclick = () => {
  if (userToDeleteId) {
    fetch(`/api/users/${userToDeleteId}`, { method: 'DELETE' })
      .then(() => { fetchUsers(); document.getElementById('delete-user-confirm-modal').style.display = 'none'; })
      .catch(err => { console.error(err); document.getElementById('delete-user-confirm-modal').style.display = 'none'; });
  }
};

function deleteUser(id) { openDeleteUserConfirmModal(id); }
...
function assignCourse(id, courseId) {
  fetch(`/api/users/${id}/assign_course`, {
    ...
    showMessage('assign-msg', msg === 'OK' ? 'Corso assegnato!' : msg, msg === 'OK' ? 'success' : 'error');
...
document.getElementById('cancel-add-user').onclick       = () => document.getElementById('add-user-modal').style.display = 'none';
document.getElementById('close-edit-modal').onclick      = () => document.getElementById('edit-user-modal').style.display = 'none';
document.getElementById('cancel-edit-user').onclick      = () => document.getElementById('edit-user-modal').style.display = 'none';
document.getElementById('close-schedules-modal').onclick = () => document.getElementById('user-schedules-modal').style.display = 'none';

window.onclick = e => {
  ['edit-user-modal','user-schedules-modal','add-user-modal','delete-user-confirm-modal'].forEach(id => {
    if (e.target === document.getElementById(id)) document.getElementById(id).style.display = 'none';
  });
  if (e.target === document.getElementById('change-role-modal')) hideChangeRoleModal();
};
...
newPassword.addEventListener('input', () => {
  ...
  addHint.textContent = missing.length ? `Mancano: ${missing.join(', ')}` : '✓ Password valida';
...
editPassword.addEventListener('input', () => {
  ...
  if (!/[a-z]/.test(v)) missing.push('minuscola');
  if (!/[0-9]/.test(v)) missing.push('numero');
  editHint.textContent = missing.length ? `Mancano: ${missing.join(', ')}` : '✓ Password valida';
...
document.getElementById('add-user-form').onsubmit = function(e) {
  ...
  fetch('/api/users', {
    ...
    if (msg === 'OK') {
      ...
      this.reset(); updateNewCourseSelect(); fetchUsers();
      setTimeout(() => document.getElementById('add-user-modal').style.display = 'none', 1400);
    } else {
...
document.getElementById('edit-user-form').onsubmit = function(e) {
  ...
  fetch(`/api/users/${editingUserId}`, {
    ...
    if (msg === 'OK') {
      ...
      fetchUsers();
      setTimeout(() => document.getElementById('edit-user-modal').style.display = 'none', 1400);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  filterRoleSelect?.addEventListener('change', () => { toggleFilterCourseVisibility(); renderUsersList(); });
  ...
  document.getElementById('refresh-data')?.addEventListener('click', () => fetchCourses().then(fetchUsers));
});
...
function updateUserStats() {
  ...
  const withCourse = users.filter(u => u.courses && u.courses.length > 0).length;
  const withoutCourse = users.filter(u => u.role === 'user' && (!u.courses || u.courses.length === 0)).length;
...
function getFilteredUsers() {
  ...
  if (course) {
    if (course === '_no_course_') f = f.filter(u => u.role === 'user' && (!u.courses || u.courses.length === 0));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  AppSocket.on('users_updated', ({ action }) => { fetchCourses().then(fetchUsers); showToast(action); });
  AppSocket.on('courses_updated', () => fetchCourses().then(fetchUsers));
});

function showToast(action) {
  const labels = {
    created:'✅ Nuovo utente creato', deleted:'🗑️ Utente eliminato',
    edited:'✏️ Utente modificato', promoted:'👑 Promosso ad admin',
    demoted:'👤 Retrocesso a utente', course_assigned:'📚 Corso aggiornato',
...
function _toast(msg) {
  ...
  t.style.cssText = 'position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;background:#1e1b4b;color:#e0e7ff;padding:.7rem 1.1rem;border-radius:12px;font-size:.8rem;font-weight:500;box-shadow:0 8px 24px rgba(0,0,0,.25);pointer-events:none;animation:_toastIn .25s ease;';
