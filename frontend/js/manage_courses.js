...
function applyFilters(courses) {
  ...
  // Ordinamento
  filtered.sort((a, b) => {
    const usersA = users.filter(u => u.courses?.some(c => c.id === a.id)).length;
    const usersB = users.filter(u => u.courses?.some(c => c.id === b.id)).length;
    if (sortOrder === 'name_asc')   return a.name.localeCompare(b.name);
    if (sortOrder === 'name_desc')  return b.name.localeCompare(a.name);
    if (sortOrder === 'users_desc') return usersB - usersA || a.name.localeCompare(b.name);
    if (sortOrder === 'users_asc')  return usersA - usersB || a.name.localeCompare(b.name);
    return 0;
  });
...
function renderCoursesList() {
  ...
  tableBody.innerHTML = filtered.map(course => {
    const enrolledUsers = users.filter(u => u.courses?.some(c => c.id === course.id));
    ...
    const descHighlighted   = highlight(descText || '<span style="font-style:italic;color:#9ca3af;">Nessuna descrizione</span>', search);
...
function updateSortIcons() {
  ...
  document.getElementById('sort-name-icon').textContent  = sort === 'name_asc' ? '↑' : sort === 'name_desc' ? '↓' : '↑↓';
  document.getElementById('sort-users-icon').textContent = sort === 'users_desc' ? '↑' : sort === 'users_asc' ? '↓' : '↑↓';
}
...
function updateActiveTags() {
  ...
  if (!container) return;
  const tags = [];
  if (search)                     tags.push(`<span class="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full font-medium">🔍 "${search}"</span>`);
  if (userFilter === 'with_users')    tags.push(`<span class="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">👥 Con iscritti</span>`);
  if (userFilter === 'without_users') tags.push(`<span class="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs rounded-full font-medium">🚫 Senza iscritti</span>`);
  if (letter)                     tags.push(`<span class="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs rounded-full font-medium">🔤 "${letter}"</span>`);
...
function updateCourseStats(filtered) {
  ...
  set('courses-with-users',    allCourses.filter(c => withUsersIds.has(c.id)).length);
  set('courses-without-users', allCourses.filter(c => !withUsersIds.has(c.id)).length);
...
document.getElementById('confirm-delete-course').onclick = () => {
  ...
  fetch(`/api/courses/${courseToDeleteId}`, { method: 'DELETE' })
    ...
    .catch(() => document.getElementById('delete-confirm-modal').style.display = 'none');
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('sort-order').addEventListener('change', renderCoursesList);
  ...
  document.querySelectorAll('.sort-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (col === 'name') {
        ...
        select.value = select.value === 'users_desc' ? 'users_asc' : 'users_desc';
      }
      renderCoursesList();
    });
  });
